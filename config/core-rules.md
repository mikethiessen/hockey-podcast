...
- Trends observable from the season game log: win/loss record, and the computed season stats provided each episode (points leaders, active point streaks, frequent scoring connections, penalty trends, movement in the productivity order, returns from absence, season-relative firsts/highs, and the season's goaltending workload, period-by-period splits, discipline over time, leads held, and shots-versus-result records) — all derived from real per-game data, never estimated
