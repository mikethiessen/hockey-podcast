# Village People — Player Notes

This file contains notes about players that the AI can reference when generating scripts.
Add nicknames, personality notes, or fun facts here. Keep it factual and friendly.
The AI will only reference these notes when the player appears in the game stats.

---

## Roster

| # | Name | Notes |
|---|------|-------|
| 4 | Ryan James | Team captain |
| 5 | Justin Gulenchyn | |
| 3 | Jaison Johnson | |
| 25 | Mike Thiessen | |
| 27 | Sergio Sousa | |
| 96 | Brastin Manningway | |
| 12 | Jaye Rynar | |
| 69 | Colin McKinley | |
| 88 | Bobby Akerolik | |
| 27 | Corey Dyck | |
| 9 | Ryan Unrau | |
| 21 | Sean Madden | |
| 11 | Michael van Reede | |
| 2 | Ian McCallister | |
| 31 | Darian Hepner | Goalie |
| 17 | Darren Stevenson | |
| 66 | Riley Goerzen | |
| 80 | Dwight Sutherland | |
| 27 | Adam Desilets | |
| 52 | Benjamin Kay | |
| 5 | Ari Kay | |
| — | David Kornas | |
| — | Eric Gagnon | Goalie (backup) |
| 31 | Sean Case | Goalie (backup) |

---

## Notes for AI
- Refer to players by first name + last name on first mention, first name only after that.
- Do not invent nicknames unless listed above.
- Do not reference jersey numbers unless adding colour (e.g. "number 11, van Reede").
- The team plays in the **Winnipeg ASHL Men's 18+ E division** (2026 Summer season).
- Home rink: **Canlan Sports Winnipeg, 1871 Ellice Ave, Rink 3**.
