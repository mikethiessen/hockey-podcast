...
Requirements:
- Follow the exact segment order from the segments config (cold_open → game_recap → player_spotlight → gord_corner → season_storylines → closing_take → next_game_preview), including any active special segments. If a "Special Segment: Guest Coach" section is present above, replace gord_corner with that guest segment for this episode only — do not include both. If a "Milestones" section is present above, weave those real facts naturally into game_recap, player_spotlight, or season_storylines — wherever they genuinely fit — rather than creating a separate segment for them.
